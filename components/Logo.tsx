...
export const Logo: React.FC<LogoProps> = ({ 
  ...
  return (
    <div className={`flex items-center gap-2.5 ${className}`}>
      {/* Premium Logo Badge */}
      <div className={`relative ${sizeClasses[size]} rounded-xl overflow-hidden shadow-sm ring-1 ring-sky-500/20 bg-gradient-to-tr from-slate-900 via-sky-950 to-slate-900 flex items-center justify-center shrink-0 group`}>
        <img 
          src={logoImg} 
      ...
      {showText && (
        <div className="flex flex-col justify-center">
          <div className="flex items-center tracking-tight">
            ...
            <span className={`${textSizes[size]} font-extrabold text-sky-600 dark:text-sky-400 tracking-tight leading-none ml-1.5`}>
              Architect
            </span>
          </div>
          {size !== 'sm' && (
